...
func redisConnFunc(network, addr string) (radix.Conn, error) {
	...
	if err != nil {
		Logger.Errorln("redis conn dial 报错！！！err:", err)
		return c, err
	}
	var ping_result string
	err = c.Do(radix.Cmd(&ping_result, "ping"))
	if err != nil {
		Logger.Errorln("redis conn do ping 报错！！！err:", err)
		return c, err
	}
	if ping_result != "PONG" {
		Logger.Errorln("redis conn do ping 没有受到PONG！！！ping_result:", ping_result)
		return c, errors.New(fmt.Sprintln("redis conn do ping 没有受到PONG！！！ping_result:", ping_result))
	}
	Logger.Debugln("连接 redis 成功！！！")
	return c, nil
}
...
func redisPoolFunc(network, addr string) (radix.Client, error) {
	...
	if err != nil {
		Logger.Errorln("redis pool dial 报错！！！errr:", err)
		return p, err
	}
	return p, nil
}
