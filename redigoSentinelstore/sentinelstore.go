...
// 创建sentine store
func NewSentinelGobStore(Sentinel *redis.Pool, sessionExpire int, keyPairs ...[]byte) *SentinleStore {
	rs := &SentinleStore{
		Sentinel: Sentinel,
...
func NewSentinelJsonStore(Sentinel *redis.Pool, sessionExpire int, keyPairs ...[]byte) *SentinleStore {
	rs := &SentinleStore{
		Sentinel: Sentinel,
		Codecs:   securecookie.CodecsFromPairs(keyPairs...),
		Options: &sessions.Options{
...
func (s *SentinleStore) load(session *sessions.Session) (bool, error) {
	...
	if r, err := redis.Bytes(c.Do("GET", s.keyPrefix+session.ID)); err != nil {
		...
		return true, s.serializer.Deserialize(r, session)
	}
}
